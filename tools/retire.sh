...
# than exec-ing into wtc-status.sh, so `ps` shows a live pane under its own
# filename now, not the one-shot script's. A `wtc-status.sh --tui` typed by
# hand still shows the latter, with --tui in argv, so both are matched.
