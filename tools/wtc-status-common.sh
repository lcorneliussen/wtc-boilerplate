...
#
# One implementation for oneshot and --tui (WTC_STATUS_UI). The previous
# dual-script shape is preserved as wtc-status-legacy.sh /
# wtc-status-legacy-tui.sh, unwired from wtc-open.sh and retire.sh.
# Tip/prod pipeline columns are hidden unless WTC_STATUS_PIPE=yes; forge
# adapters beyond GitHub are stubs in lib.sh. Snapshots write
# .wtc-status.json/.md and .last-wtc-status.yml.
...
# someone else's collection, borrow that collection's harness/ so status can
# still resolve the registry — and infer WTC_HARNESS_REPO from the
# harness_repo_bare_owner reason line when unset.
...
# so a --cached reader that only ever saw the yml still has something to
# render. Cast from the CR_* arrays load_snapshot already built; nothing
