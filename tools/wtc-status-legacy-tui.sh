#!/usr/bin/env bash
# wtc-status-legacy-tui.sh — the live status pane, boilerplate-native impl.
#
# Kept for side-by-side testing against the Steep-ported wtc-status.sh /
# wtc-status-tui.sh (see harness/tools/steep-status*.sh and
# port/status-from-steep). Not wired into wtc-open.sh or retire.sh any more —
# run it by hand to compare.
#
# A two-line exec onto `wtc-status-legacy.sh --tui`, and deliberately nothing more.
...
exec "$script_dir/wtc-status-legacy.sh" --tui --repos "$@"
